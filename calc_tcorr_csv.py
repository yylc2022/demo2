...
def generate_html_report_csv(output_dir, report_name,
                             t1, t2, t3, t3plus, t4, t5, t5plus, t6, t7, t7plus, t8plus, t6plus, ic_raw_data=None,
    ...
    if HAS_BOKEH:
        try:
            ...
            if ic_raw_data is not None and not ic_raw_data.empty:
                try:
                    ...
                    if len(data) > 1:
                        ...
                        # 3. Normal Distribution Curve (Scaled)
                        x_curve = np.linspace(min(data), max(data), 1000)
                        pdf = 1 / (sigma * np.sqrt(2 * np.pi)) * np.exp(-(x_curve - mu) ** 2 / (2 * sigma ** 2))

                        # Scale PDF to match counts: pdf * N * bin_width
                        bin_width = edges[1] - edges[0]
                        total_count = len(data)
                        pdf_scaled = pdf * total_count * bin_width
                        ...
                        p.line(x_curve, pdf_scaled, line_color="#ff8888", line_width=3, alpha=0.8,
                               legend_label="正态分布拟合曲线")
            ...
            if not t8plus.empty:
                plot_list.append(
                    Div(text="<div style='font-weight: 600; font-size: 16px; margin: 20px 0 10px;'>3.5 Benchmark Cumulative Return</div>",
                        sizing_mode='stretch_width'))
                plot_list += fb.plot('Label Cumulative Return (Benchmark)', [('line', t8plus)])
...
def calc_tcorr_from_csv(
    ...
    if all_results:
        ...
        # Table 8 (Label Accumulation)
        try:
            print("\n--- Generating minbar_table8 (Label Accumulation) ---")
            tab8 = minbar_table8_csv(raw_data_dict, 'label', target_symbol=target_symbol)
            print("\n[minbar_table8]")
            print(tab8)
            tab8.to_csv(os.path.join(output_dir, 'minbar_table8.csv'))
        except Exception as e:
            print(f"Error table8: {e}")
        ...
        try:
            ...
            generate_html_report_csv(output_dir, "tcorr_report",
                                     ...
                                     tab8 if 'tab8' in locals() else pd.DataFrame(),
...
if __name__ == "__main__":
    signal_file = os.path.join('data', 'predictions.csv')
    label_file = os.path.join('data', 'true_values.csv')

    #  在这里修改 target_symbol 来选择你想要的品种
    calc_tcorr_from_csv(signal_file, label_file, target_symbol='000905.SH')
